...
import { ArrowRight, Calendar, Loader2, MapPin, Ticket, Users } from "lucide-react";
...
export default function EventsPage() {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([api.getEvents(), api.getPublicSettings()])
      .then(([events, publicSettings]) => {
        if (!active) return;

        const typedEvents = events as EventItem[];
        const typedSettings = publicSettings as { whatsapp_group_url: string };
        const now = new Date();
        setUpcoming(typedEvents.filter((event) => new Date(event.date) >= now));
        setPast(typedEvents.filter((event) => new Date(event.date) < now));
        setWhatsappGroupUrl(typedSettings.whatsapp_group_url);
      })
      .catch((error) => {
  ...
  const handleRsvpSubmit = async (e: import("react").FormEvent<HTMLFormElement>) => {
    e.preventDefault();
      ...
      <Dialog open={Boolean(selectedEvent)} onOpenChange={(open) => !open && closeRsvp()}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>Fill the details below to confirm your attendance.</DialogDescription>
          </DialogHeader>
