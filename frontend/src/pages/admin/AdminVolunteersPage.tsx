...
import type { AdminVolunteerRow } from "../../types/api";

export function AdminVolunteersPage() {
  const { data, loading, error } = useApiData<AdminVolunteerRow[]>("/volunteers/admin/list", []);

  return (
    <div>
      <p className="text-xs font-bold uppercase tracking-[0.2em] text-trust-700">Admin module</p>
      <h1 className="mt-3 font-display text-4xl text-ink">Volunteer requests</h1>
      <p className="mt-3 text-sm leading-7 text-ink/65">
        Review people who have raised their hand to volunteer through the public site.
      </p>

      <div className="mt-8 rounded-[30px] border border-white bg-white p-6 shadow-panel">
        <AsyncState error={error} loading={loading} loadingLabel="Loading volunteer requests..." />
        {!loading && !error ? (
          data.length ? (
            <div className="overflow-hidden rounded-3xl border border-trust-100">
              <table className="min-w-full divide-y divide-trust-100 text-sm">
                <thead className="bg-trust-50 text-left text-trust-700">
                  <tr>
                    {["Name", "Email", "Phone", "City", "Interest area", "Status", "Received"].map(
                      (heading) => (
                ...
                <tbody className="divide-y divide-trust-100 bg-white">
                  {data.map((row) => (
                    <tr key={row.id}>
                      ...
                      <td className="px-4 py-4">{row.interest_area ?? "-"}</td>
